$(function() {
    ...
    for (let i = 0; i < inputArray.length; i++) {
        for (let j = 0; j < inputArray.length; j++) {
            if ((inputArray[i] + inputArray[j] == targetNumber) && !pOneDone) {
                pOneDone = true;
                console.log('Problem 1');
                console.log(`Nr 1: ${inputArray[i]}, Nr 2: ${inputArray[j]}`);
                console.log('Svar: ' + inputArray[i] * inputArray[j])
            }
            for (let k = 0; k < inputArray.length; k++) {
                if (inputArray[i] + inputArray[j] + inputArray[k] == targetNumber && !pTwoDone) {
                    pTwoDone = true;
                    console.log('Problem 2');
                    console.log(`Nr 1: ${inputArray[i]}, Nr 2: ${inputArray[j]} , Nr 3: ${inputArray[k]}`);
                    console.log('Svar: ' + inputArray[i] * inputArray[j] * inputArray[k]);
                }
            }
        }
    }

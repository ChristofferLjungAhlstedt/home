$(function() {
    ...
    function parseImportantLetter(condition, passCode) {
        ...
        let timesAllowed = condition[0];
        let keyLetter = condition[1];
        timesAllowed = timesAllowed.split('-');
        let allowedMin = timesAllowed[0];
        let allowedMax = timesAllowed[1];
        allowedMin = parseInt(allowedMin);
        allowedMax = parseInt(allowedMax);
            let re = new RegExp(keyLetter, 'g');
        try {
            let count = passCode.match(re).length; 
            checkIfPasscodePass(count, allowedMin, allowedMax);
        } catch (err) {
            count = 0;
            checkIfPasscodePass(count, allowedMin, allowedMax);
        }
    ...
    function checkIfPasscodePass(count, allowedMin, allowedMax) {
        if ((count <= allowedMax) && (count >= allowedMin)) {
            passwordMeetStandard = passwordMeetStandard + 1;
            return
        }
    }

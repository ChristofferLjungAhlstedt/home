$(function() {
    ...
    function splitItem() {
        ...
        for (let i = 0; i < inputArray.length; i++) {
            activeItem = inputArray[i];
            activeItem = activeItem.split(':');
            condition = activeItem[0];
            passCode = activeItem[1];
            parseImportantLetter(condition, passCode);
        }
        console.log(passwordMeetStandard);
    }

    function parseImportantLetter(condition, passCode) {
        condition = condition.split(' ');
        let position = condition[0];
        let keyLetter = condition[1];
        position = position.split('-');
        let positionOne = position[0];
        let positionTwo = position[1];
        positionOne = parseInt(positionOne);
        positionTwo = parseInt(positionTwo);
        checkIfPasscodePass(keyLetter, positionOne, positionTwo, passCode);
    }

    function checkIfPasscodePass(keyLetter, positionOne, positionTwo, passCode) {
        console.log(passCode.includes(keyLetter, positionOne), passCode.includes(keyLetter, positionTwo));
        if ((passCode.includes(keyLetter, positionOne - 1)) && !(passCode.includes(keyLetter, positionTwo - 1))) {
            passwordMeetStandard++;
            return
        }
        if (!(passCode.includes(keyLetter, positionOne)) && (passCode.includes(keyLetter, positionTwo))) {
            passwordMeetStandard++;
            return
        }
    }
